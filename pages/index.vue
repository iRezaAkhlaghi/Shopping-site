<template>
    ...
    <div class="container p-4">
        ...
        <div v-show="activateButton === 1"
            class="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 sm:gap-4">
            <div v-for="d in data" :key="d.id"
                class=" my-2 relative max-w-md mx-auto bg-white rounded-md overflow-hidden shadow-md transition-transform transform hover:scale-105">
                <img class="w-full h-48 object-cover rounded-t-md" :src="d.image" alt="Product Image" />

                <div class="p-4">
                    <h2 class="sm:text-xl text-base font-semibold text-gray-800">{{ d.title }}</h2>
                    <p class="text-gray-600 sm:text-base text-sm line-clamp-3 mb-6">{{ d.description }}</p>
        ...
        <div v-show="activateButton === 2" class="mt-10 flex flex-col  justify-start gap-4">
            <div v-for="d in data" :key="d.id"
                ...
                <div class="p-4 w-full">
                    <h2 class="sm:text-xl text-sm font-semibold text-gray-800">{{ d.title }}</h2>
                    <p class="text-gray-600 sm:text-base text-xs line-clamp-3 mb-6">{{ d.description }}</p>
                   
                    <div class="flex items-center justify-between">

                        <p class="text-purple-600 font-semibold sm:text-base text-xs  ">{{ d.price }} $</p>
                        <p @click="addToCart(d)"
                            class="text-purple-600 font-semibold sm:text-base text-xs  hover:text-purple-800 hover:scale-105 cursor-pointer ">
                            Add To Cart </p>
